...
@pytest.mark.asyncio
async def test_verify_returns_claims_for_a_valid_token(
...
@pytest.mark.asyncio
async def test_verify_returns_none_for_unknown_token(
    verifier: personal_token.PersonalTokenVerifier,
) -> None:
...
@pytest.mark.asyncio
async def test_verify_returns_none_for_revoked_token(
    verifier: personal_token.PersonalTokenVerifier,
    tokens: identity_service.PersonalTokenService,
    identity: identity_service.IdentityService,
) -> None:
...
@pytest.mark.asyncio
async def test_verify_returns_none_when_the_user_has_been_deleted(
    verifier: personal_token.PersonalTokenVerifier,
    tokens: identity_service.PersonalTokenService,
    identity: identity_service.IdentityService,
    users: in_memory_identity_store.InMemoryUserStore,
) -> None:
    """A valid, unexpired token whose owning user no longer exists must not
    verify — the user could have been deleted after the token was minted.
    """
    user = await identity.resolve_or_create("entra-oid-123")
    _, secret = await tokens.mint(user.user_id, "Claude Code")
