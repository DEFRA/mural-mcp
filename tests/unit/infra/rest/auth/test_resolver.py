...
@pytest.mark.asyncio
async def test_trusted_header_resolver_creates_a_user_on_first_sight(
...
@pytest.mark.asyncio
async def test_trusted_header_resolver_is_stable_across_calls(
    identity: identity_service.IdentityService,
) -> None:
    resolver_under_test = resolver.TrustedHeaderUserResolver("X-User-Id", identity)
...
@pytest.mark.asyncio
async def test_trusted_header_resolver_requires_the_header(
    identity: identity_service.IdentityService,
) -> None:
    resolver_under_test = resolver.TrustedHeaderUserResolver("X-User-Id", identity)
...
@pytest.mark.asyncio
async def test_personal_token_resolver_builds_principal_from_claims() -> None:
    fake = _FakeVerifier(
...
@pytest.mark.asyncio
async def test_personal_token_resolver_requires_bearer_header() -> None:
...
@pytest.mark.asyncio
async def test_personal_token_resolver_rejects_an_unverifiable_token() -> None:
    resolver_under_test = resolver.PersonalTokenUserResolver(_FakeVerifier(None))
