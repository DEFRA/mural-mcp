...
@pytest.mark.asyncio
async def test_get_valid_token_returns_access_token_when_not_expired(fake_config):
...
@pytest.mark.asyncio
async def test_get_valid_token_returns_access_token_when_expiry_unknown(fake_config):
    token = schemas.MuralToken(
        access_token="example-access-token",
        refresh_token="example-refresh-token",
        expires_at=None
...
@pytest.mark.asyncio
async def test_get_valid_token_refreshes_and_stores_when_expired(fake_config):
    past_exp = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=1)
    new_token_data = {"access_token": "example-new-access-token", "refresh_token": "example-new-refresh-token"}

    old_token = schemas.MuralToken(
        access_token="example-access-token", refresh_token="example-refresh-token", expires_at=past_exp
    )
